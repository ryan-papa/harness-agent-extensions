...
| `rp-youtube-channel` | 채널의 공개 영상을 최신순으로 훑어 **장표로 만들 수 있는 후보만** 추림. 이미 만든 영상(장표의 `deck-source-ref` 해시 메타)과 멤버십·자막없음으로 걸러진 영상을 건너뛰고 남은 것만 순차 프로브. 상태는 장표 레포에 두어 기기가 바뀌어도 유지. 터미널 표 + 후보 JSON 출력 |
