...
상태는 대상 루트(장표 레포)에 둔다. 로컬에만 두면 기기·캐시 초기화로 유실된다.
  ...
  <root>/**/*.html 의 deck-source-video     이미 장표로 만든 영상 (복구 가능한 이중화)
...
def published_video_ids(root):
    """장표 HTML의 deck-source-video 메타를 훑어 이미 만든 영상 ID를 모은다."""
    done = {}
    pat = re.compile(r'name="deck-source-video"\s+content="([A-Za-z0-9_-]{11})"')
...
def main():
    ...
    for vid, title in videos:
        if len(candidates) >= args.limit or probes >= args.max_probe:
            break
        if vid in done or vid in excluded or vid in deferred:
            continue
    ...
    if args.json:
        ...
        print()
        print(f"프로브 {probes}건" + (f" · 제목 규칙으로 건너뜀 {skipped_title}건" if skipped_title else ""))
